...
export default function InvoiceWorkflow() {
  ...
  const handleUpload = async () => {
    ...
    // Simulate upload
    setTimeout(() => {
      // Create new invoice entries from uploaded files
      const newInvoices = files.map((file, index) => ({
        id: `new-${Date.now()}-${index}`,
        category: '-',
        description: file.name.replace('.pdf', '').replace(/-/g, ' '),
        vendor: '-',
        amount: '-',
      ...
      setProcessedData(prev => [...prev, ...newInvoices]);
      setFiles([]);
      setIsUploading(false);
      setIsDialogOpen(false);
      
      toast.success(`${newInvoices.length} invoice(s) uploaded`);
    }, 1500);
  };
            ...
            <CardContent className="p-6">
              <div className="flex justify-between items-center mb-4">
                <div className="flex items-center">
                  ...
                  {unprocessedCount > 0 && (
                    <Badge className="ml-2 bg-blue-100 text-blue-800">{unprocessedCount} unprocessed</Badge>
                  )}
                </div>
